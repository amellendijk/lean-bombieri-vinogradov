...
theorem parseval_trig_poly_complex (a : ℤ → ℂ) (S : Finset ℤ) :
  ...
  have hcont_inner : ∀ m : ℤ, Continuous (fun x =>
      ∑ n ∈ S, starRingEnd ℂ (a m) * starRingEnd ℂ (e (↑m * x)) *
        (a n * e (↑n * x))) := by
    intro m; apply continuous_finset_sum; intro n _
    exact (continuous_const.mul (continuous_e_mul _).star).mul
      (continuous_const.mul (continuous_e_mul _))
  ...
  rw [intervalIntegral.integral_finset_sum (fun n _ => hint m n)]
...
theorem parseval_trig_poly_norm (a : ℤ → ℂ) (S : Finset ℤ) :
  ...
  have hT_cont : Continuous T := continuous_finset_sum S (fun n _ =>
    continuous_const.mul (continuous_e_mul _))
  ...
  -- Step 3: ∫ re(f) = re(∫ f)
  have hre : (∫ x in (0:ℝ)..1, (starRingEnd ℂ (T x) * T x).re) =
      (∫ x in (0:ℝ)..1, starRingEnd ℂ (T x) * T x).re := by
...
theorem continuous_trig_poly (a : ℤ → ℂ) (S : Finset ℤ) :
  ...
  continuous_finset_sum S (fun n _ => continuous_const.mul (continuous_e_mul _))
